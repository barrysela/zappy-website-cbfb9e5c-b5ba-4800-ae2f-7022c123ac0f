...
if (!window.zappyContactFormLoaded) {
  ...
  try {
    ...
    const zappyResponse = await fetch('https://api.zappy5.com/api/email/contact-form', {
      ...
      body: JSON.stringify({
        websiteId: 'cbfb9e5c-b5ba-4800-ae2f-7022c123ac0f',
        name: data.name || '',
        email: data.email || '',
    ...
    if (zappyResponse.ok) {
      ...
      console.log('⚠️ Zappy: Server returned non-OK status', zappyResponse.status);
    }
  } catch (error) {
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Assi Delivery Service משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
